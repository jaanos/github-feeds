...
$(function() {
  ...
  $.when.apply($, ajax).done(function() {
    ...
    for (i = keys.length-1; i >= 0; i--) {
      ...
      if (["PushEvent", "IssuesEvent", "PullRequestEvent",
           "CommitCommentEvent", "IssueCommentEvent"].indexOf(result.type) != -1) {
        ...
        if (result.type == "PushEvent") {
          ...
          if (result.payload.commits.length == 0) {
            commits.className = "message";
            commits.innerHTML = "<blockquote>" + makeLink(result, "https://github.com/" + result.repo.name + "/tree/" + branch, branch, "branch") + " is now at " + makeLink(result, "https://github.com/" + result.repo.name + "/commit/" + result.payload.head, result.payload.head.substring(0, 7), "commit") + "</blockquote>";
          } else {
        ...
        if (result.type == "CreateEvent") {
          ...
          action = "deleted " + result.payload.ref_type + " <span class=\"branch-name\">" + result.payload.ref + "</span> at " + makeLink(result, "https://github.com/" + result.repo.name, result.repo.name, "repo")
          ...
          action = "forked " + makeLink(result, "https://github.com/" + result.repo.name, result.repo.name, "repo") + " to " + makeLink(result, result.payload.forkee.html_url, result.payload.forkee.full_name, "fork")
        }
